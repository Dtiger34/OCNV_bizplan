...
Chạy lệnh coverage của layer được chọn, xem báo cáo:
...
- Mock: `PrismaService`, external services
...
- Mock: API calls (TanStack Query), router

Không test: `components/ui/*` (shadcn re-exports), `main.tsx`, `router/index.tsx`
...
- **BE**: chạy Jest với flag `--coverage --coverageReporters=html lcov` → output vào `docs/coverage/be/`
- **FE**: chạy Vitest với `--coverage` → output vào `docs/coverage/fe/`

Verify file `docs/coverage/be/index.html` và `docs/coverage/fe/index.html` tồn tại trước khi báo done.
