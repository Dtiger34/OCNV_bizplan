        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-4 reveal-left">
            {cartItems.map((item) => (
              <div
                key={item.id}
                className="flex flex-col sm:flex-row gap-4 p-4 bg-[#fff8e7] border border-[#D4B896] rounded-[6px] relative group hover:border-[#C9973A] transition-all"
              >
                <img
                  src={item.image}
                  alt={item.name}
                ...
                <div className="flex-1 flex flex-col justify-between">
                  ...
                  <div className="flex items-center justify-between mt-4">
                    {/* Quantity Selector */}
                    <div className="flex items-center border border-[#D4B896] rounded-sm bg-[#fff8e7]">
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity - 1)}
                        className="p-1.5 text-[#ab2124] hover:bg-[#ab2124]/5 transition-colors"
                      >
                        <Minus size={14} />
                      </button>
                      <span className="px-4 text-sm text-[#ab2124]">
                        {item.quantity}
                      </span>
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity + 1)}
